#!/usr/bin/env python3
"""
say-last-response.py - Read Claude's last response aloud via OpenAI TTS.

Backs the `/say` slash command. You listen to long responses instead of reading
them. Manual trigger only - nothing auto-fires.
...
What it does:
  ...
  5. Write the mp3 to a STABLE path. On macOS, when local (not SSH) with mpv
     present, AUTOPLAY it by opening a NEW Terminal window (via `open`) running
     mpv - a real terminal so your speed/seek/pause keys reach the player. A
     player launched detached takes no key presses; a real window does. The
     window comes from `open`, not AppleScript: a slash command runs inside a
     sandbox that blocks AppleEvents, but `open -a Terminal <file>.command` is
     allowed. Over SSH (no local display), without mpv, or off macOS, it falls
     back to printing the play command. The stable path also makes over-SSH
     playback work: pull the file to your laptop and play it there.

API key: read from $OPENAI_API_KEY, else ~/.config/claude-voice/openai-key. No
...
Env overrides: SAY_TTS_MODEL (default gpt-4o-mini-tts), SAY_TTS_VOICE (alloy).

Stdlib only (urllib for HTTP). Exit codes: 0 = ok, 1 = user-facing error.
"""
...
# a NEW Terminal window so you get a keyboard for speed/seek/pause controls - a
# detached player can never take key presses, but a real terminal window can.
# The stable path also makes over-SSH playback work: pull THIS file to your
# laptop and play it there.
...
# get a real terminal window (AppleScript control of a terminal is blocked by
# the sandbox a slash command runs in).
PLAY_LAUNCHER = CONFIG_DIR / "say-play.command"

TTS_URL = "https://api.openai.com/v1/audio/speech"
DEFAULT_MODEL = os.environ.get("SAY_TTS_MODEL", "gpt-4o-mini-tts")
DEFAULT_VOICE = os.environ.get("SAY_TTS_VOICE", "alloy")
...
def read_api_key():
    """Return the OpenAI key from env or the local secret file, or None."""
...
def ssh_server_ip():
    ...
    SSH_CONNECTION is "client_ip client_port server_ip server_port"; the laptop
    reaches THIS host at server_ip to pull the audio.
    """
...
def autoplay_in_terminal(path):
    """Autoplay mpv in a NEW Terminal window so your keys reach the player.

    A detached player takes no key presses; a real terminal window does. The
    window comes from `open` (LaunchServices), NOT AppleScript: a slash command
    runs in a sandbox that blocks AppleEvents, but `open -a Terminal
    <file>.command` is allowed and Terminal executes the .command. macOS + local
    + mpv only: over SSH there is no local display, and without mpv there is no
    controllable player - both keep the printed-instructions fallback.
    ...
    if not mpv or not shutil.which("open"):
        return False
    # Ensure the config dir exists; this function must not depend on write_audio
    # having run first (a fresh install has no ~/.config/claude-voice yet).
...
def play_instructions(minutes, autoplayed=False):
    """Build the status line + play commands.
    ...
    autoplay did not fire (SSH, no mpv, off macOS, or --no-play). mpv is the only
    common player that does speed+seek+pause; ffplay is a no-speed fallback.
    """
    lines = []
    if autoplayed:
        lines.append(
...
def stop_playback():
    ...
    Autoplay runs mpv inside a Terminal window you quit with q; this is a
...
def main():
    ...
    if unknown:
        # Reject before any synthesis. An unrecognized arg must never fall
        # through to a real (paid) OpenAI call.
    ...
    # Default autoplays in a real Terminal window so your keys drive the player;
    # --no-play suppresses the window (synthesize + write only).
